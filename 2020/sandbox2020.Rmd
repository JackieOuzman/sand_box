...
## Build on this I want now to say if you select mixing what type of mixing do you want to do?
...
output$tb_chosen <- renderTable(subset(df,
 df$grouping==input$choose_grouping & 
 df$intervenion==input$choose_item), rownames = TRUE)
...
## Build on this now I want now to say if you select mixing what type of mixing do you want to do and what site is it most like.

```{r select_nested_grouping2,echo=FALSE }
...
inputPanel(
 ...
 selectInput(inputId="choose_item2",
            label="Select item2",
...
output$tb_chosen2 <- renderTable(subset(df,df$grouping==input$choose_grouping2 & df$intervenion==input$choose_item2), rownames=TRUE)
...
#  first selectInput is changed
 observe({ updateSelectInput(session,
                            inputId="choose_item2", choices=unique(df[df$grouping == input$choose_grouping2,"intervenion"])
                            )
        })
